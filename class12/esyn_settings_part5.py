# This file contains code that will create ball and stick neurons in a ring that have both
# excitatory and inhibitory syns and NetCons, along with an AlphaSynapse to allow for input current
# for the simulation
...
# Creation of the excitatory and inhitbitory Syns and NetCons to be utilized in the simulation
Esyn = []
Isyn = []
ENetCon = []
INetCon = []
for i in range(N):
    ...
    isyn = u.createEsyn(synapse_location = target.dend(0.5), erev = -80, tau = 0.1)
    Inetcon = u.connectNetStim(stim = source.soma(0.5)._ref_v, synapse = isyn, delay = 5, 
			       weight = 0.02, section = source.soma)
    Isyn.append(isyn)
